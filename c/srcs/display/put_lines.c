...
void put_lines(t_data* data, t_vertex** copy) {
    ...
    while (y < data->map.height) {
        ...
        while (x < data->map.width) {
            ...
            if (x + 1 < data->map.width) v1 = create_int_vertex(&copy[y][x + 1]);
            if (x + 1 < data->map.width) bresenham(data, &v0, &v1);
            v0 = create_int_vertex(&copy[y][x]);
            if (y + 1 < data->map.height) v1 = create_int_vertex(&copy[y + 1][x]);
            if (y + 1 < data->map.height) bresenham(data, &v0, &v1);
            ++x;
        }
        ++y;
    }
