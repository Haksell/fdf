...
void init_params(t_data* data) {
    ...
    data->params.scale =
        (0.5 * get_min(WINDOW_WIDTH, WINDOW_HEIGHT) /
...
bool init_minilibx(t_data* data, char* window_title) {
    data->mlx = mlx_init();
    if (data->mlx == NULL) return (false);
    data->win = mlx_new_window(data->mlx, WINDOW_WIDTH, WINDOW_HEIGHT, window_title);
    if (data->win == NULL) return (false);
    data->img = mlx_new_image(data->mlx, WINDOW_WIDTH, WINDOW_HEIGHT);
    if (data->img == NULL) return (false);
    ...
    if (data->addr == NULL) return (false);
    return (true);
}
...
t_vertex** init_vertices(int width, int height) {
    ...
    while (i < height) {
        ...
        if (vertices[i] == NULL) {
            ft_free_double((void***)&vertices, i);
            return (NULL);
        }
        ++i;
    }
